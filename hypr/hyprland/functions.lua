local function wsaction(action, range, i)
    return function()
        ...
        if activews then
            ...
            -- Group mode: (target_group - 1) * 10 + (active % 10), with 0 mapped to 10
            local rel_ws = id % 10
            if rel_ws == 0 then rel_ws = 10 end
            local group_target = (i - 1) * 10 + rel_ws

            -- Workspace mode: floor((active - 1) / 10) * 10 + target
            local ws_target = math.floor((id - 1) / 10) * 10 + i

            local z = (range == "group") and group_target or ws_target
